...
const Sales = () => {
  ...
  // Estados locales para inputs de descuento AISLADOS POR SESIÓN
  const [discountAmountInput, setDiscountAmountInput] = useState('');
  const [discountPercentInput, setDiscountPercentInput] = useState('');

  // CRÍTICO: Resetear inputs cuando cambie la sesión activa
  useEffect(() => {
    const activeSession = sessions[activeSessionId];
    if (activeSession) {
      // Cargar valores específicos de la sesión activa
      setDiscountAmountInput(activeSession.discountAmount > 0 ? activeSession.discountAmount.toString() : '');
      setDiscountPercentInput(activeSession.discountPercent > 0 ? activeSession.discountPercent.toString() : '');
    } else {
      // Resetear si no hay sesión activa
